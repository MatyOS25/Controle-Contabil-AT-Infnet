package br.edu.infnet.financialcontrol.repository;
import br.edu.infnet.financialcontrol.model.domain.Lancamento;
import br.edu.infnet.financialcontrol.model.domain.User;
import java.util.ArrayList;
import java.util.List;
import java.util.Map;  
import java.util.HashMap;
import java.util.Collection;

public class LancamentosRepository {
    private static Integer id = 1;
    private static Map<Integer,Lancamento> mappedEntity = new HashMap<Integer, Lancamento>();
    public static boolean add(Lancamento entity){
        try{
            entity.setId(id++);
            mappedEntity.put(entity.getId(),entity);
            return true;
        }catch(Exception e){
            return false;
        }
    }
    public static Lancamento remove(Integer key){
      return mappedEntity.remove(key);  
    }
    public static Collection<Lancamento> returnList(){
        return  mappedEntity.values();
    }
}
